// droplet: smallest Liquid template engine for JavaScript (ES2020, async-only)
((root) => {
  ...
  const BUILTIN_FILTERS = {
    ...
    find: (v, k, val) => { const a = arr(v); const ks = k.split("."); return a.find(x => { let o = x; for (const s of ks) o = o?.[s]; return val !== undefined ? o === val : truthy(o); }); },
    find_index: (v, k, val) => { const a = arr(v); const ks = k.split("."); const idx = a.findIndex(x => { let o = x; for (const s of ks) o = o?.[s]; return val !== undefined ? o === val : truthy(o); }); return idx >= 0 ? idx : null; },
    ...
    sum: (v, k) => { const flat = a => { const r = []; for (const x of arr(a)) isArr(x) ? r.push(...flat(x)) : r.push(x); return r; }; const toN = x => { const raw = k ? x?.[k] : x; if (raw == null || typeof raw === "boolean" || typeof raw === "object") return 0; const n = +raw; return isNaN(n) ? 0 : n; }; let af = false; const items = isArr(v) ? flat(v) : [v]; const r = items.reduce((s, x) => { const it = k ? x?.[k] : x; if (it?.__f || (it != null && typeof it !== "boolean" && (""+it).includes("."))) af = true; return s + toN(x); }, 0); const rv = +r.toPrecision(12); return af && rv % 1 === 0 ? rv.toFixed(1) : rv; },
  ...
  if (typeof module !== "undefined" && module.exports) module.exports = Droplet;
  else if (typeof define === "function" && define.amd) define(() => Droplet);
  else root.Droplet = Droplet;
